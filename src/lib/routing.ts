...
async function fetchNudged(
  ...
  const res = await fetchWithTimeout(
    `foot/${coords}?overview=full&geometries=geojson`
  );
 ...
 *
 * Always makes exactly 2 parallel requests:
 *   1. Direct A→B with alternatives=3
 *   2. Nudged A→WP→B (perpendicular waypoint) — guarantees a different path
 * This ensures two visually distinct routes in all cases.
 */
export async function fetchSafeAndFastRoutes(
  ...
  // Two parallel requests — main direct + nudged alternative
  ...
  // Gather all candidate safe routes: natural alternatives + nudged route
  ...
  // Prefer natural alternatives with low backtracking; nudge is always a valid fallback
  const cleanNatural = naturalAlts.filter(c => c.bt < 0.3).sort((a, b) => a.bt - b.bt);
  ...
  // Nothing useful found — show single route (rare)
